...
PATH_HDF5_DIR = os.path.join(settings.base_dir, 'tmp', 'hdf5')
...
class TripletLoss(object):
    ...
    def triplet_loss(self, inputs: list, dist='sqeuclidean', margin='maxplus'):
    ...
    def convert_to_tflite(self, embedding_model) -> None:
    ...
    def predict(self, file_paths: list, spot_id: int, is_exhibit: bool) -> None:
        """Predict triplet newwork
        ...
        file_paths: list
            file path of S3 resource
        spot_id: int
            spot_id is required to save the csv file for each sightseeing spot
        is_exhibit: bool
